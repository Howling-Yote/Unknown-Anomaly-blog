...
// Submit guestbook entry to Firebase
  ...
  if (guestbookForm) {
    guestbookForm.addEventListener('submit', function(event) {
      ...
      if (typeof firebase === 'undefined' || !firebase.database) {
        alert('Error: Firebase not loaded properly!');
        return;
      }
...
function loadGuestbookEntries() {
  ...
  if (typeof firebase === 'undefined' || !firebase.database) {
    entriesContainer.innerHTML = '<p style="color: #ff0000;">Error: Firebase not loaded properly!</p>';
    return;
  }
